...
#        Frim[t,:] = data[col][(t)*nk:(t+1)*nk, 7]
    ...
    im5 = ax[5].contourf(range(nt), z_agl, np.transpose(zet), levels=levs_Z, colors=ccol_zet)
    fig.colorbar(im5, ax=ax[5], shrink=0.8, pad=cb_pad)
...
fig, ax = plt.subplots(nrows=6, ncols=1, figsize=(9,14), sharex=True)
