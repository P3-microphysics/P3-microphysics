...
!
! This program creates the lookup table 'p3_lookupTable_1.dat' for inter-category ice-ice
...
!--------------------------------------------------------------------------------------------
!# Parallel script 2 (of 3):   [copy text below (uncommented) to file 'go_2-submit.csh']
!#  - creates individual work directories, launches each executable

!#!/bin/csh
!
! foreach exec (`ls exec_*`)
! 
...
!--------------------------------------------------------------------------------------------
 ...
 integer, parameter :: n_Fr      =   4   !formerly rimsize
 integer, parameter :: n_Qnorm   =  25   !formerly isize
 ...
 real :: N,q,qdum,dum1,dum2,cs1,ds1,lam,n0,lamf,qerror,del0,c0,c1,c2,dd,sum1,sum2,       &
         ...
         dcritr,csr,dsr,duml,dum3,rhodep,cgpold,m1,m2,m3,dt,mur,initlamr,lamv,           &
...
! array for RIME density (not ice/graupel density)
 crp(1) =  50.*pi*sxth
 crp(2) = 250.*pi*sxth
 crp(3) = 450.*pi*sxth
 crp(4) = 650.*pi*sxth
...
! case of partial riming (Fr between 0 and 100%)
       ...
       elseif (i_Fr.eq.2.or.i_Fr.eq.3) then
          ...
          do
             ...
             dcritr1(i_rhor,i_Fr) = ((1.+Fr(i_Fr)/(1.-Fr(i_Fr)))*cs/cgp1(i_rhor))**(1./(dg-ds))
             csr1(i_rhor,i_Fr)    = cs*(1.+Fr(i_Fr)/(1.-Fr(i_Fr)))
             dsr1(i_rhor,i_Fr)    = ds
...
! density of vapor deposition/aggregation grown ice
             cgpold = cgp1(i_rhor)
             cgp1(i_rhor) = crp(i_rhor)*Fr(i_Fr)+rhodep*(1.-Fr(i_Fr))*pi*sxth
...
!            lam1(i_rhor,i_Fr,i) = 1.0013**ii*10.   ! new lambda_i limiter
             ...
             mu_i1 = diagnostic_mui(log_diagmu_orig,lam1(i_rhor,i_Fr,i),q,cgp1(i_rhor),Fr(i_Fr),pi)               
...
! case of partial riming (Fr between 0 and 100%)
       elseif (i_Fr.eq.2.or.i_Fr.eq.3) then
          ...
          do
             ...
             dcritr2(i_rhor,i_Fr) = ((1.+Fr(i_Fr)/(1.-Fr(i_Fr)))*cs/cgp2(i_rhor))**(1./(dg-ds))
             csr2(i_rhor,i_Fr)    = cs*(1.+Fr(i_Fr)/(1.-Fr(i_Fr)))
             dsr2(i_rhor,i_Fr)    = ds
! get mean density of vapor deposition/aggregation grown ice
...
! density of vapor deposition/aggregation grown ice
             cgpold = cgp2(i_rhor)
             cgp2(i_rhor) = crp(i_rhor)*Fr(i_Fr)+rhodep*(1.-Fr(i_Fr))*pi*sxth
...
! cover full range over mean size from 2 to 5000 micron
          ...
          ii_loop_2: do ii = 1,9000
             ...
             mu_i2 = diagnostic_mui(log_diagmu_orig,lam2(i_rhor,i_Fr,i),q,cgp1(i_rhor),Fr(i_Fr),pi)
