...
!!$         if (associated(zkmid)) cnv_active = cnv_active + zkmid
         ...
         if (p3_trplmomi) then
           call conv_mp_tendencies1(zprctns, zpritns, ttp, qcp, ncp, qip, nip, qti1p, nti1p, ztdmask, ni, nk, nkm1, zti1p)
         else
      ...
      if (p3_trplmomi) then
        call conv_mp_tendencies1(zprcten, zpriten, ttp, qcp, ncp, qip, nip, qti1p, nti1p, ztdmask, ni, nk, nkm1, zti1p)
      else
     ...
     MIDLEVEL_CONVECTION: if (conv_mid == 'KF') then
      ...
      if (p3_trplmomi) then
        call conv_mp_tendencies1(zprctnm, zpritnm, ttp, qcp, ncp, qip, nip, qti1p, nti1p, ztdmask, ni, nk, nkm1, zti1p)
      else
